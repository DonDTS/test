...
{
    global $sn, $u, $cf;
    // Меняем шрифт font-size!
    if ($i == 0 && !XH_ADM) {
        if ($x == '' && $cf['locator']['show_homepage'] == 'true') {
            return '<a style="font-size: 18px" href="' . $sn . '?' . $u[0] . '"' . (!empty($class_etc) ? ' ' . $class_etc : '') . '>'; // Для главного пункта меню
        }
    }
    return isset($u[$i])
        ? '<a style="font-size: 18px" href="' . $sn . '?' . $u[$i] . $x . '"' . (!empty($class_etc) ? ' ' . $class_etc : '') . '>' // Для всех остальных
...
//			}else if($k == 1){
//	            $lines[] = "\n" . '<ul class="' . 'dropdown-submenu once' . '">' . "\n";
			}else{
	            $lines[] = "\n" . '<ul class="' . 'dropdown-menu once ' .  $this->st . ($k + 1) . '">' . "\n";
			}
...
//		if($i == 3){ //MenuLelel3
		if($this->getMenuLevel($i) == 2 && $this->hasChildren($i)){
			$className = 'dropdown-submenu once';
		}else{
	        $className = '';
